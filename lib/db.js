...
var DB = module.exports = function (pathToDb) {
  this.db = pathToDb ? levelup(pathToDb) : defaultDb;
};

DB.prototype = {
  get: function (key, _cb) {
    return this.db.get(key, function (err, data) {
      if (err || !data) return _cb(err);
      return _cb(null, deserializeData(data));
    });
  },
  put: function (key, val, _cb) {
    return this.db.put(key, serializeData(val), function (err) {
      return _cb(err);
    });
  },
  putMerge: function (key, val, _cb) {
    if (!_.isObject(val)) {
      return _cb(new Error('merging value is not an object!'));
    }
    var self = this;
    return this.db.get(key, function (err, extantStr) {
      if (!extantStr) {
        self.put(key, val, _cb);
      } else {
        var extant = deserializeData(extantStr);
        if (!extant || !_.isObject(extant)) {
          return _cb(new Error('existing data is not an object!'));
        }
        return self.put(key, _.extend(extant, val), _cb);
      }
    });
  }
};
